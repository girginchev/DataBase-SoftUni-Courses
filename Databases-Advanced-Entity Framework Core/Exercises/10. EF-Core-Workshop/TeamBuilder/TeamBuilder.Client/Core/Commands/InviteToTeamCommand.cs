        ...
        public string Execute(string[] inputArgs)
        {
            ...
            using (var db = new TeamBuilderContext())
            {
                ...
                if (loggedUser.UserId != team.CreatorId &&
                    team.UserTeams.All(x => x.UserId != loggedUser.UserId)
                    || team.UserTeams.Any(x => x.UserId == userToInvite.UserId))
                {
                    throw new InvalidOperationException("Not allowed!");
                }

                bool alreadyInvited = db.Invitations.Any(x => x.InvitedUserId == userToInvite.UserId && x.TeamId == team.TeamId && x.IsActive);
